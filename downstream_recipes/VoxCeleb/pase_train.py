...
signal_workers = ['F0semitoneFrom27.5Hz_sma3nz_amean',
       ...
       'StddevUnvoicedSegmentLength', "mfcc"]
...
class PASE(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        for signal_worker in signal_workers : 
            if signal_worker in self.considered_workers : 
                if signal_worker not in ["Loudness_sma3","mfcc", "melfs", "F0final_sma",
    ...
    def compute_objectives(self, predictions, batch,other_targets, stage):
        ...
        target_melf, target_mfcc = other_targets
        self.workers_target={"melfs" : target_melf, "mfcc": target_mfcc}
        exoworkers = [x for x in self.considered_workers if x not in ["melfs",
                                                                 "mfcc"]]
...
#        target_loudness = F.pad(input=target_loudness ,
#                                pad=(1,1),mode='constant',value=0)[:, :-1]
    ...
    def fit_batch(self, batch):
        ...
        predictions, feats,  mfcc = self.compute_forward(batch, sb.Stage.TRAIN)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        predictions,feats,  mfcc = self.compute_forward(batch, stage=stage)
        other_targets = [feats, mfcc]
        with torch.no_grad():
...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(wav):
        ...
        return resampled
    needed_workers = [x for x in hparams["workers"] if x not in ["melfs", "mfcc"]]
